...
export default function ParentParticlesBackground() {
  ...
  useEffect(() => {
    ...
    // Parent-themed emojis
    const parentEmojis = ['👨‍👩‍👧‍👦', '🏡', '🧒', '📞', '❤️', '📝'];

    // Respect reduced motion
    const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    // Particle configuration (kept consistent with student version)
    ...
    const createParticles = () => {
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        particlesRef.current.push({
          ...
          vx: initialVx,
          vy: initialVy,
          emoji: parentEmojis[Math.floor(Math.random() * parentEmojis.length)],
    ...
    // Debounced recreation to prevent stalls during rapid resize/scroll
    let recreateTimeout: number | undefined;
    const scheduleRecreate = () => {
      if (recreateTimeout) window.clearTimeout(recreateTimeout);
      recreateTimeout = window.setTimeout(() => {
        createParticles();
      }, 150);
    };
    ...
    const resizeCanvas = () => {
      ...
      // Avoid immediate particle re-creation on every resize event
      scheduleRecreate();
    };
    resizeCanvas();
    // Ensure initial particles exist promptly
    createParticles();
